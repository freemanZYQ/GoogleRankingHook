...
public class NetWorkUtil {
    ...
    public static String getNetIp(Context context) {
        try {
//            String infoUrl = "http://ip.taobao.com/service/getIpInfo2.php?ip=myip";
//            HttpCache httpCache = CacheManager.getHttpCache(context);
//            HttpResponse response = httpCache.httpGet(infoUrl);
//            if (response != null) {
//                JSONObject result = JSONUtils.toJsonObject(response.getResponseBody());
//                JSONObject data = JSONUtils.getJSONObject(result, "data", null);
//                String ip = JSONUtils.getString(data, "ip", "");
//                return ip;
//            }
            String ipfoUrl = "https://api.ipify.org";
            HttpCache httpCache = CacheManager.getHttpCache(context);
            HttpResponse response = httpCache.httpGet(ipfoUrl);
            if(response != null){
    ...
    public  static String  getIpCountry(Context context,String ip){
        try {
            String URL = "http://ip.taobao.com/service/getIpInfo.php?ip=%s";
            URL = String.format(URL, ip);
            HttpCache httpCache = CacheManager.getHttpCache(context);
            HttpResponse response = httpCache.httpGet(URL);
            if (response != null) {
                JSONObject result = JSONUtils.toJsonObject(response.getResponseBody());
                JSONObject data = JSONUtils.getJSONObject(result, "data", null);
                String country = JSONUtils.getString(data, "country", "");
                return country;
            }
        }catch (Exception e){
            e.printStackTrace();
...
//		ToastUtil.showMessage(context, "无法连接网络");
    ...
    public static String getImei() {
        try {
            ...
            if (imei == null) {
                getImeiByReflect(applicationContext);
            }
            return imei;
        } catch (Exception e) {
            e.printStackTrace();
