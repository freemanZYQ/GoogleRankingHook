...
final class d extends SQLiteOpenHelper {
    ...
    public d(Context context, String str) {
        ...
        if (v.b()) {
            am.h.a("SQLiteOpenHelper " + this.a);
        }
    }
        ...
        try {
            ...
            try {
                ...
                while (query.moveToNext()) {
                    arrayList.add(new e(query.getLong(0), query.getString(1), query.getInt(2), query.getInt(3)));
                }
                ContentValues contentValues = new ContentValues();
                for (e eVar : arrayList) {
                    contentValues.put("content", r.b(eVar.b));
                    sQLiteDatabase.update("events", contentValues, "event_id=?", new String[]{Long.toString(eVar.a)});
                }
                if (query != null) {
                    query.close();
                }
    ...
    public final void onUpgrade(SQLiteDatabase sQLiteDatabase, int i, int i2) {
        am.h.f("upgrade DB from oldVersion " + i + " to newVersion " + i2);
        if (i == 1) {
            sQLiteDatabase.execSQL("create table if not exists keyvalues(key TEXT PRIMARY KEY NOT NULL, value TEXT)");
            a(sQLiteDatabase);
            b(sQLiteDatabase);
